...
export default {
	...
	countdown(time1, time2) {
		...
		let d, h, m, s
		d = parseInt(dayjs(time1).diff(dayjs(time2), 'day'))
		h = parseInt(dayjs(time1).diff(dayjs(time2), 'hour') % 24)
		m = parseInt(dayjs(time1).diff(dayjs(time2), 'minute') % 60)
		s = parseInt(dayjs(time1).diff(dayjs(time2), 'second') % 60 % 60)
		console.log([d, h, m, s]);
		return [d, h, m, s]
	},
	// 判断是否是当天
